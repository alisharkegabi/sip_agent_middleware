...
async def transfer_call_by_tracking_id(tracking_id: str):
    """Same as POST /calls/{call_id}/transfer, but keyed by tracking_id --
    the id ElevenLabs already has in dynamic_variables mid-call, since it
    was supplied by the original caller in the /calls request rather than
    generated by this service. Configure the agent's "transfer_call" webhook
    tool URL as POST {this service}/calls/by-tracking-id/{{tracking_id}}/transfer."""
    manager = get_manager()
